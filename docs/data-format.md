...
percentiles and total bytes, and counts of `slots` (rows plus skip events), skipped slots, paused
rounds and degraded rounds. Outage thresholds are left to analysis; every figure is recomputable
from the samples.
...
| `short` | bytes crossed, over a span too brief to divide by |
...
`resting`, `unsupported`, `short` and `error` are excluded from failure tallies: each describes the
tool or the browser.
...
| `round_ms` `phase_idle_ms` `phase_down_ms` | the round's wall time, and its two phases: the idle probes, then the download with the round trip taken across it. `phase_down_ms` includes up to 1 s of the refusal check after a `network` failure. A frozen tab suspends the abort timers, so a round can outlast every deadline in it |
...
| `grades` | the three activity grades as displayed. `null` for the object: the round threw. `null` for one activity: unrated, because a required term has no measurement |
...
| `blocked` | `ip6` `ip4` | this family carried traffic in the round and the literal address was refused. Excluded from tallies |
...
| `ms_samples` `samples_ok` `ms_min` `ms_max` | `ip6` `ip4` `dns` `dns_ctl` `web` `udp` | every latency sample, the success count, and the spread the median omits; `ms` is the median of the successful samples |
...
| `stall_check` | `down` | present when a stream was still waiting for headers 2 s in: `same_host`, `other_host` and `udp`, each `{ok, ms, fail}`. Download host slow with gstatic fast: browser or host. Both slow: link. UDP fast with both slow: TCP path |
...
`timing-allow-origin`; the other five endpoints return zeroed timing cross-origin. `handshake`
...
| `note` | free text and recorder notices: wake lock lost or regained, position accuracy class change, probe rested, egress address change under an unchanged operator label |
| `skip` | a slot came due `late_ms` behind schedule while round `round` was still running: `running_ms` so far, and `waiting_on`, the unsettled probe ids (`loaded_rtt` for the loaded round trip) |
